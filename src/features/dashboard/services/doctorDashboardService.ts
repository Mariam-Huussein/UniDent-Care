...
export const doctorDashboardService = {
    ...
    getDoctorRequests: async (page: number = 1, pageSize: number = 10): Promise<PaginatedRequests> => {
        const response = await api.get(`/Doctors/my-requests?page=${page}&pageSize=${pageSize}`);
        return response.data.data;
    },
