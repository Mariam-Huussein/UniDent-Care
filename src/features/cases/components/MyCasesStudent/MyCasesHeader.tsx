import { Briefcase, Send, LayoutGrid, List, Loader2 } from "lucide-react";
...
export default function MyCasesHeader({
    ...
    casesLoading, casesViewMode, setCasesViewMode, requestsViewMode, setRequestsViewMode
}: {
    activeTab: string;
    ...
    return (
        <div className="relative z-10 rounded-3xl bg-white dark:bg-slate-900/50 border border-slate-100 dark:border-slate-800 shadow-2xl shadow-slate-200/50 dark:shadow-none p-6 sm:p-8 mb-8 overflow-hidden backdrop-blur-xl transition-all duration-300">
            ...
            {/* Main Wrapper: Changed flex-col to lg:flex-row to align side-by-side on desktop */}
            <div className="relative flex flex-col lg:flex-row lg:items-center justify-between gap-6">
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <p className="text-slate-500 dark:text-slate-400 text-xs sm:text-sm font-medium">
                                {activeTab === "cases" ? "Manage your registered cases." : "Track your sent requests."}
                            </p>
                        </div>
                    </div>
                ...
                {/* Removed border-t and pt-6 to keep it on the same line */}
                <div className="flex flex-wrap items-center gap-3">
                            ...
                            return (
                                ...
                                >
                                    ...
                                    <span className="relative z-10 flex items-center gap-2">
                                        <tab.icon size={14} className={isActive ? "text-indigo-600 dark:text-indigo-400" : "opacity-60"} />
                                        {tab.label}
                    ...
                    {/* Vertical Divider - Hidden on very small screens if needed */}
                    ...
                    <div className="flex bg-slate-100 dark:bg-slate-950/50 p-1 rounded-xl ring-1 ring-slate-200 dark:ring-slate-800">
                        <button
                            ...
                            className={`p-2 rounded-lg transition-all ${currentViewMode === 'grid' ? 'bg-white dark:bg-slate-800 text-indigo-600 shadow-sm ring-1 ring-slate-200 dark:ring-slate-700' : 'text-slate-400 hover:text-slate-600'}`}
                        >
                            <LayoutGrid size={18} />
                        </button>
                        <button
                            onClick={() => setViewMode('table')}
                            className={`p-2 rounded-lg transition-all ${currentViewMode === 'table' ? 'bg-white dark:bg-slate-800 text-indigo-600 shadow-sm ring-1 ring-slate-200 dark:ring-slate-700' : 'text-slate-400 hover:text-slate-600'}`}
                        >
                            <List size={18} />
                        </button>
                    </div>
