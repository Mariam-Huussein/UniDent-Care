  ...
  const signupMutation = useMutation({
    ...
    onError: (err: any) => {
      ...
      if (serverErrors) {
        ...
        const msg =
          apiError?.message ||
          (isRtl ? "حدث خطأ ما" : "Something went wrong during registration");
        toast.error(msg);
      }
    },
  });
        ...
        <div className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl rounded-[2.5rem] shadow-xl dark:shadow-[0_32px_64px_-12px_rgba(0,0,0,0.5)] border border-white dark:border-slate-800 p-8 sm:p-12 transition-all duration-300">
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-5">
            ...
            <div className="space-y-1.5">
              ...
              {errors.phone && <p className="text-xs text-red-500 dark:text-red-400 font-bold">{errors.phone.message}</p>}
            </div>
